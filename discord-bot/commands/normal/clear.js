...
module.exports = {
    ...
    if (!message.channel.permissionsFor(message.member).has("MANAGE_MESSAGES")) return message.reply("```Insfufficient permissions for you```")
    if (!message.channel.permissionsFor(message.guild.me).has("MANAGE_MESSAGES")) return message.reply("```Insufficient permissions for me```");
